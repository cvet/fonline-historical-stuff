...
const BYTE COND_NOT_IN_GAME			=4;//состояние смерти
...
const BYTE OBJ_PIC_INV			=17;//{Индекс картинки в инвентаре}
const BYTE OBJ_SOUND			=18;//{Индекс звука}

const BYTE OBJ_LIVETIME			=19;//{Начальное время жизни}

const BYTE OBJ_COST				=20;//{Базовая цена}
const BYTE OBJ_MATERIAL			=21;//{Материал предмета}
//----------------------------------------------- type==OBJ_TYPE_ARMOR
...
const BYTE OBJ_ARM_AC			=34;//{Армор класс}
const BYTE OBJ_ARM_PERK			=35;//{Перк на броне}

const BYTE OBJ_ARM_DR_NORMAL	=36;//{Сопротивление нормальное}
const BYTE OBJ_ARM_DR_LASER		=37;//{Сопротивление лазеру}
const BYTE OBJ_ARM_DR_FIRE		=38;//{Сопротивление огню}
const BYTE OBJ_ARM_DR_PLASMA	=39;//{Сопротивление плазме}
const BYTE OBJ_ARM_DR_ELECTR	=40;//{Сопротивление электричеству}
const BYTE OBJ_ARM_DR_EMP		=41;//{Сопротивление емп}
const BYTE OBJ_ARM_DR_EXPLODE	=42;//{Сопротивление взрыву}

const BYTE OBJ_ARM_DT_NORMAL	=43;//{Порог повреждения нормальное}
const BYTE OBJ_ARM_DT_LASER		=44;//{Порог повреждения лазеру}
const BYTE OBJ_ARM_DT_FIRE		=45;//{Порог повреждения огню}
const BYTE OBJ_ARM_DT_PLASMA	=46;//{Порог повреждения плазме}
const BYTE OBJ_ARM_DT_ELECTR	=47;//{Порог повреждения электричеству}
const BYTE OBJ_ARM_DT_EMP		=48;//{Порог повреждения емп}
const BYTE OBJ_ARM_DT_EXPLODE	=49;//{Порог повреждения взрыву}
...
//принодлежность к игроку
	DWORD player;
	BYTE active;
//принодлежность к тайлу
	WORD map;
	HexTYPE x;
	HexTYPE y;
//объект-контейнер
...
//////////////////////////////////////////////////////////////////////////
// поворот криттера по часовой
// params:
// CrID id

#define NETMSG_CRITTER_ROTCCW 12
//////////////////////////////////////////////////////////////////////////
// поворот криттера против часовой
// params:
// CrID id
...
// params:
// DWORD idobj; -ид объекта (если 0, то скрывание объекта)
#define NETMSG_SEND_USE_OBJECT 25 //!Cvet
//////////////////////////////////////////////////////////////////////////
// сигнал чузена о приминении активного предмета
// params:
// BYTE type - тип объекта на который используется предмет
// DWORD id; - ID объекта-цели
// BYTE ori; - напрвление применения
//
...
// CrID id; -другого игрока (если себя то свой ИД)
// HexTYPE tile_x; -тайл Х
// HexTYPE tile_y; -тайл У
