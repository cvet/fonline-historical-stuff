...
int CHexField::ParseScenObj(DWORD proto_id,DWORD id,DWORD x,DWORD y,WORD* hbmax,WORD* hemax,WORD* wrmax,WORD* wlmax)
{
	...
	case SP_SCEN_BLOCK:
	case SP_SCEN_IBLOCK:
		hex_field[y][x].passed=FALSE; //!Cvet проходимость
	case SP_SCEN_LIGHT:
	case SP_SCEN_MARKER:
...
int CHexField::ParseWallObj(DWORD proto_id,DWORD id,DWORD x,DWORD y,WORD* hbmax,WORD* hemax,WORD* wrmax,WORD* wlmax)
{
	...
	hex_field[y][x].passed=FALSE; //!Cvet проходимость
	hex_field[y][x].raked=FALSE; //!Cvet простреливаемость
...
{
// Походу если 0 то переменная приравнивается 1 иначе в 0
	ShowHex=ShowHex?0:1;
...
int CHexField::FindStep(HexTYPE start_x, HexTYPE start_y, HexTYPE end_x, HexTYPE end_y)
{
	...
	while(true)
	{
		...
		else return FINDPATH_DEADLOCK; //индекс не увеличился - значит тупик
...
int CHexField::FindTarget(HexTYPE start_x, HexTYPE start_y, HexTYPE end_x, HexTYPE end_y, BYTE max_weapon_distance)
{
	...
	while(true)
	{
		...
		numindex++;
		//проверяем дистанцию
		if(numindex-1 > max_weapon_distance) return FINDTARGET_TOOFAR; //смотрим дистанцию оружия
	...
	while(numindex>2)
	{
		...
		return FINDTARGET_BARRIER; //найдена преграда на пути выстрела
	}
